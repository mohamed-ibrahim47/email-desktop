...
public class MessageRendererService extends Service<Void>{
	...
	public MessageRendererService(WebEngine messageRendererEngine) {
		...
		this.setOnSucceeded(x ->{showMessage();});
	}
	...
	private void renderMessage(){
		...
		Message message = messageToRender.getMsg();
		try {
	...
	/*
	//avoid runin on another thread not on main application thread 
	@Override
	public void run() {
		renderMessage();
	...
	*/
	...
	// show msg only when it's loades (to work on the main thread so the UI dont freeze )
